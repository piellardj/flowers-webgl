...
__webpack_require__(/*! ../page-interface-generated */ "./src/ts/page-interface-generated.ts");
function parseRGB(hexaColor) {
    var redHex = hexaColor.substring(1, 3);
    var greenHex = hexaColor.substring(3, 5);
    var blueHex = hexaColor.substring(5, 7);
    return {
...
var PlotterCanvasWebGL = (function (_super) {
    ...
    function PlotterCanvasWebGL() {
        ...
        _this.stemsVBOId = gl_canvas_1.gl.createBuffer();
        _this.petalsVBOId = gl_canvas_1.gl.createBuffer();
        ...
        ShaderManager.buildShader({
            ...
            if (builtShader === null) {
                var errorMessage = "Failed to load or build the stems shader.";
                Page.Demopage.setErrorMessage("shader-stems", errorMessage);
                throw new Error(errorMessage);
            }
            _this.stemsShader = builtShader;
        });
        ShaderManager.buildShader({
            ...
            if (builtShader === null) {
                var errorMessage = "Failed to load or build the petals shader.";
                Page.Demopage.setErrorMessage("shader-petals", errorMessage);
                throw new Error(errorMessage);
            }
            _this.petalsShader = builtShader;
        });
        ShaderManager.buildShader({
            ...
            if (builtShader === null) {
                var errorMessage = "Failed to load or build the corolla shader.";
                Page.Demopage.setErrorMessage("shader-corolla", errorMessage);
                throw new Error(errorMessage);
            }
            _this.corollaShader = builtShader;
        });
        return _this;
    }
    PlotterCanvasWebGL.prototype.initialize = function (backgroundColor) {
        ...
        gl_canvas_1.gl.clearColor(rgb.r, rgb.g, rgb.b, 1);
    ...
    PlotterCanvasWebGL.prototype.drawLines = function (lines, color) {
        if (this.stemsShader && lines.length > 0) {
            ...
            var buffer = new Float32Array(2 * 2 * totalNbLines);
            var i = 0;
            ...
            gl_canvas_1.gl.bindBuffer(gl_canvas_1.gl.ARRAY_BUFFER, this.stemsVBOId);
            gl_canvas_1.gl.bufferData(gl_canvas_1.gl.ARRAY_BUFFER, buffer, gl_canvas_1.gl.DYNAMIC_DRAW);
            var aVertexLoc = this.stemsShader.a["aVertex"].loc;
            gl_canvas_1.gl.enableVertexAttribArray(aVertexLoc);
            gl_canvas_1.gl.vertexAttribPointer(aVertexLoc, 2, gl_canvas_1.gl.FLOAT, false, 0, 0);
            gl_canvas_1.gl.drawArrays(gl_canvas_1.gl.LINES, 0, 2 * totalNbLines);
        }
    };
    ...
    PlotterCanvasWebGL.prototype.drawEllipsis = function (ellipsis, color) {
        var substring = color.substring(5, color.length - 1);
        var channelsAsString = substring.split(", ");
        var colorRgb = {
    ...
    PlotterCanvasWebGL.prototype.drawPolygonsBatch = function () {
        if (this.corollaShader && this.polygonsBatch.length > 0) {
            var totalNbVertices = 0;
            var totalNbTriangles = 0;
            var totalNbLines = 0;
            for (var _i = 0, _a = this.polygonsBatch; _i < _a.length; _i++) {
                var polygon = _a[_i];
            ...
            for (var _b = 0, _c = this.polygonsBatch; _b < _c.length; _b++) {
                var polygon = _c[_b];
                var polygonDepth = 1 - iPolygon / this.polygonsBatch.length;
                ...
                for (var _d = 0, _e = polygon.outline; _d < _e.length; _d++) {
                    var outlinePoint = _e[_d];
            ...
            for (var _f = 0, _g = this.polygonsBatch; _f < _g.length; _f++) {
                var polygon = _g[_f];
            ...
            iVerticeIndex = 0;
            for (var _h = 0, _j = this.polygonsBatch; _h < _j.length; _h++) {
                var polygon = _j[_h];
                for (var iPoint = 0; iPoint < polygon.outline.length - 1; iPoint++) {
            ...
            var aDataLoc = this.corollaShader.a["aData"].loc;
    ...
    PlotterCanvasWebGL.prototype.drawEllipseBatches = function () {
        if (this.petalsShader && this.ellipseBatches.length > 0) {
            var totalNbPoints = 0;
            for (var _i = 0, _a = this.ellipseBatches; _i < _a.length; _i++) {
                var ellipseBatch = _a[_i];
                totalNbPoints += ellipseBatch.ellipseList.length;
            }
            var i = 0;
            var buffer = new Float32Array(8 * totalNbPoints);
            for (var _b = 0, _c = this.ellipseBatches; _b < _c.length; _b++) {
                var ellipseBatch = _c[_b];
                for (var _d = 0, _e = ellipseBatch.ellipseList; _d < _e.length; _d++) {
                    var ellipse = _e[_d];
            ...
            gl_canvas_1.gl.bindBuffer(gl_canvas_1.gl.ARRAY_BUFFER, this.petalsVBOId);
            gl_canvas_1.gl.bufferData(gl_canvas_1.gl.ARRAY_BUFFER, buffer, gl_canvas_1.gl.DYNAMIC_DRAW);
            var aData1Loc = this.petalsShader.a["aData1"].loc;
            ...
            var aData2Loc = this.petalsShader.a["aData2"].loc;
